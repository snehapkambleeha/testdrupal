...
class DomainSourceTokenTest extends DomainTestBase {
  ...
  public function testDomainSourceTokens() {
    ...
    $this->assertEquals($domains['one_example_com']->getPath() . 'node/1', $token_handler->replace('[node:canonical-source-domain-url]', ['node' => $node]));
    $this->assertEquals($domains['one_example_com']->getPath() . 'node/1', $node->toUrl('canonical')->setAbsolute()->toString());

    $node->set(DomainSourceElementManagerInterface::DOMAIN_SOURCE_FIELD, 'two_example_com')->save();
    $this->assertEquals($domains['two_example_com']->getPath() . 'node/1', $token_handler->replace('[node:canonical-source-domain-url]', ['node' => $node]));
    $this->assertEquals($domains['two_example_com']->getPath() . 'node/1', $node->toUrl('canonical')->setAbsolute()->toString());
    ...
    // Re-fetch the token handler from the rebuilt container.
    $token_handler = \Drupal::token();

    // Test token value, and URL without token.
    $node->set(DomainSourceElementManagerInterface::DOMAIN_SOURCE_FIELD, 'one_example_com')->save();
