...
- **Domain**
  ...
  administrators, provides a Block and Views display context, and creates
...
The Domain module automatically adds `domain` to the
`required_cache_contexts` via its service provider. This
ensures that Drupal's render cache varies by domain. No
manual configuration of `services.yml` is needed.

Note: older versions of Domain required manually adding
`url.site` to `required_cache_contexts` in your
`services.yml`. This is no longer necessary and any manual
override can be safely removed.
...
For Drupal 10+, you can use the [Domain DDEV](https://github.com/agentrickard/domain-ddev) project for
getting started quickly. It includes all the tools described below.

If you file a pull request or patch, run the existing tests to check for
failures. Writing additional tests will greatly speed completion, as I won't
commit code without test coverage.

New tests should be written in PHPUnit as Functional, FunctionalJavascript,
Kernel, or Unit tests.

To setup a proper environment locally, you need multiple or wildcard domains
