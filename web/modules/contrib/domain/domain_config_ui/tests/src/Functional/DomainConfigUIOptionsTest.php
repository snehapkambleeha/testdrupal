...
class DomainConfigUIOptionsTest extends DomainConfigTestBase {
  ...
  /**
   * Tests access the settings form.
   */
  public function testFormOptions() {
    ...
    $domains =
    \Drupal::entityTypeManager()->getStorage('domain')->loadMultiple();
    ...
    $string = 'value="' . $domain->id() . '"';
    if (in_array(
    $domain->id(), ['two_example_com', 'three_example_com'], TRUE)) {
    $this->assertSession()->responseContains($string);
    }
    else {
    $this->assertSession()->responseNotContains($string);
  ...
  /**
   * Tests hook to disable settings form.
   */
  public function testDisallowedRoutesHook() {
    $this->drupalLogin($this->adminUser);
    $path = 'admin/config/people/accounts';

    // Visit the disabled path via hook.
    $this->drupalGet($path);
