...
class DomainHookTest extends KernelTestBase {
  ...
  public function testHookDomainLoad() {
    ...
    $path = $domain->getPath();
    $url = $domain->getUrl();
    $this->assertNotEmpty($path, new FormattableMarkup('The path property was set to %path.', ['%path' => $path]));
    $this->assertNotEmpty($url, new FormattableMarkup('The url property was set to %url.', ['%url' => $url]));

    // External hooks.
    $this->assertEquals('bar', $domain->get('foo'), 'The foo property was set to <em>bar</em> by hook_domain_load.');
  }
  ...
  public function testHookDomainValidate() {
    ...
    $this->assertEquals('Fail.example.com cannot be registered', current($errors), 'Error message returned correctly.');
  }
  ...
  public function testHookDomainRequestAlter() {
    ...
    $this->assertEquals('bar1', $domain->get('foo1'), 'The foo1 property was set to <em>bar1</em> by hook_domain_request_alter');
  }
  ...
  public function testHookDomainReferencesAlter() {
    ...
    $this->assertEquals('Test string', $query->getMetaData('domain_test'), 'Domain test query altered.');
