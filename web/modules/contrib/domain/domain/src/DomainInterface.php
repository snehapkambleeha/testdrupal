...
interface DomainInterface extends ConfigEntityInterface {
  ...
  /**
   * Gets the base path for a domain without the path prefix.
   *
   * @return string
   *   A URL string for the base path to the domain,
   *   without the path prefix. (e.g. http://example.com/)
   */
  public function getBasePath();

  /**
   * Gets the path for a domain, including the path prefix.
   *
   * @return string
   *   A URL string for the base path to the domain,
   *   including the path prefix if set.
  ...
  /**
   * Returns a generated link for a domain.
   *
   * The link text is the canonical hostname. When the domain has a
   * path prefix, it is appended (e.g. "example.com/fr").
   *
   * @param bool $current_path
   *   Indicates the link should point to the current request path.
   *
   * @return \Drupal\Core\GeneratedLink
   *   A generated link object.
   */
  ...
  /**
   * Gets the path prefix for this domain.
   *
   * @return string
   *   The path prefix without leading or trailing slashes,
   *   or an empty string if no prefix is set.
   */
  public function getPathPrefix(): string;

  /**
   * Sets the path prefix for this domain.
   *
   * @param string $prefix
   *   The path prefix without leading or trailing slashes.
   *
  ...
  /**
   * Matches this domain's prefix against a request path.
   *
   * Handles both raw UTF-8 and percent-encoded non-ASCII
   * prefixes. Returns the byte length of the matched prefix
   * segment (including the leading slash) so the caller can
   * strip or inspect it, or FALSE if the path does not start
   * with this domain's prefix.
   *
   * @param string $path
   *   The raw request path (e.g. from getPathInfo()).
   *
   * @return int|false
   *   The byte length of the prefix in the path (including
   *   leading slash), or FALSE if no match.
   */
