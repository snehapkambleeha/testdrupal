...
class DomainValidator implements DomainValidatorInterface {
   ...
   * @see https://www.drupal.org/node/3574840
   */
  public function validate($hostname) {
    @trigger_error('DomainValidator::validate() is deprecated in domain:3.0.0 and is removed from domain:4.0.0. Use entity constraint validation instead. See https://www.drupal.org/node/3574840', E_USER_DEPRECATED);
    $error_list = [];
  ...
  public function checkResponse(DomainInterface $domain) {
    $url = $domain->getBasePath() . $this->moduleHandler->getModule('domain')->getPath() . '/tests/200.png';
    try {
