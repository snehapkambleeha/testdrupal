...
#[Block(
  id: 'domain_nav_block',
  admin_label: new TranslatableMarkup('Domain navigation'),
)]
class DomainNavBlock extends DomainBlockBase {
  ...
  public function getCacheContexts() {
    ...
    // cached by domain if we are printing the homepage path, not the request.
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $elements['link_options'] = [
      ...
      '#default_value' => $this->configuration['link_options'] ?? $defaults['link_options'],
    ...
    $elements['link_theme'] = [
      '#type' => 'radios',
      '#title' => t('Link theme'),
      '#default_value' => $this->configuration['link_theme'] ?? $defaults['link_theme'],
      '#options' => $options,
      '#description' => $this->t('Select how to display the block output.'),
    ];
    $options = [
      'name' => $this->t('The domain display name'),
      'hostname' => $this->t('The raw hostname (with path prefix if set)'),
      'url' => $this->t('The domain base URL'),
    ];
    $elements['link_label'] = [
      '#type' => 'radios',
      '#title' => $this->t('Link text'),
      '#default_value' => $this->configuration['link_label'] ?? $defaults['link_label'],
      '#options' => $options,
  ...
  public function build() {
    ...
    foreach ($this->domainStorage->loadMultipleSorted() as $domain) {
      ...
      // Handles menu links.
      if ($domain->status() || $this->currentUser->hasPermission('access inactive domains')) {
